...
export function ShareButtons({ title, slug }: Props) {
  ...
  const url = `https://zabanion.ir/blog/${slug}`;
  ...
  return (
    <div className="flex items-center gap-1.5">
      <button onClick={copyLink}
        className="flex-1 py-2 rounded-xl border border-stone-200 dark:border-white/[0.06] text-[10px] font-medium text-stone-500 hover:border-amber-300 hover:text-amber-500 transition-all flex items-center justify-center gap-1">
        {copied ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
        {copied ? "کپی شد" : "کپی"}
      </button>
      <button onClick={shareTwitter}
        className="p-2 rounded-xl bg-[#1DA1F2]/10 text-[#1DA1F2] hover:bg-[#1DA1F2]/20 transition-all">
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </button>
      <button onClick={shareTelegram}
        className="p-2 rounded-xl bg-[#0088cc]/10 text-[#0088cc] hover:bg-[#0088cc]/20 transition-all">
